...
class WatchController extends Controller
{
     ...
     */
     public function category_list($id){
        ...
        $id = Auth::id(); //get authenticated user id
        $user = User::find($id); // get authenticated user
        return view('category_list', compact('watches', 'categories', 'title', 'user'));
    ...
    */
    ...
    public function search()
    {
        ...
        if(!$watches->isEmpty()){
            $id = Auth::id(); //get authenticated user id
            $user = User::find($id); // get authenticated user
            return view('/shop_search', compact('watches','user','search_term'))->with('success', 'Results found by searchterm - ' . $search_term); 
        }else{
            return redirect()->back()->with('error', 'Sorry, nothing found by searchterm - ' . $search_term);
        }
